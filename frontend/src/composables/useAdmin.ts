...
const adminViewPaths = {
  ...
  codes: `${managementEntryPath}/codes`,
  redeem: `${managementEntryPath}/redeem`,
  pools: `${managementEntryPath}/pools`,
  settings: `${managementEntryPath}/settings`,
} satisfies Record<AdminView, string>
...
export async function refreshAdmin() {
  await withBusy(async () => {
    await loadPools()
    if (activeView.value === 'pools' || activeView.value === 'redeem') {
      return
    }
...
function initialAdminView(): AdminView {
  return viewFromPath(window.location.pathname)
    || viewFromValue(window.location.hash)
...
function viewFromPath(pathname: string): AdminView | null {
  const normalized = normalizePath(pathname)
  if (normalized === managementEntryPath) return 'accounts'
  if (!normalized.startsWith(`${managementEntryPath}/`)) return null
  const segment = normalized.slice(managementEntryPath.length + 1).split('/')[0]
  return viewFromValue(segment)
}

export function isManagementPath(pathname = window.location.pathname) {
  const normalized = normalizePath(pathname)
  return normalized === managementEntryPath || normalized.startsWith(`${managementEntryPath}/`)
}
